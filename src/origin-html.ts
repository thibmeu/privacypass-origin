...
<script type="module">
  window.extension_supported = false

  // try to retrieve PrivacyPassExtensionV from Server-Timing header
    ...
    if ( navTiming.length > 0 ) {
      ...
      if ( serverTiming && serverTiming.length > 0 ) {
        for ( let i=0; i < serverTiming.length; i++ ) {
          if ( serverTiming[ i ].name == 'PrivacyPassExtensionV' && serverTiming[ i ].description.indexOf( '4' ) == 0 )
            window.extension_supported = true
        }
      }
    }
  }
   
  if ( window.extension_supported ) {
    document.getElementById( 'a1' ).classList.remove( 'hidden' )
    document.getElementById( 'snt' ).classList.remove( 'hidden' )
    document.getElementById( 'b2' ).firstElementChild.classList.add( 'green' )
    document.getElementById( 'b3' ).firstElementChild.classList.add( 'green' )
  }
  else
    document.getElementById( 'e1' ).classList.remove( 'hidden' )

  console.log( 'window.extension_supported: ' + window.extension_supported )
...
<div class="description">
  <br>
  <div class="current" id="b1"><span class="num">1</span>Origin returns WWW-Authenticate header</div>
  <div class="action hidden" id="a1">Browser extension opens tab to Attester</div>
  <div class="error hidden" id="e1">Missing <a href="https://chrome.google.com/webstore/detail/privacy-pass/ajhmfdgkijocedmfjonnpjfojldioehi">browser extension</a></div>
...
<main>
  <br>
  <h2><a class="q" id="h" target="_blank" href="https://blog.cloudflare.com/eliminating-captchas-on-iphones-and-macs-using-new-standard/">Privacy Pass Token</a> &#128721; not presented</h2>
  <br>
